...
const umbrielIndex = uranus.moons.indexOf("Umbriel");
if (umbrielIndex !== -1) {
  uranus.moons[umbrielIndex] = "Oberon";
}

// 8. Iterate through the solarSystem array and print only the objects that 
//have a ringSystem (where ringSystem: true), and ignore the others.
...
const system = {
  ...
  // 3. console.log the sum of the number of satellites of Jupiter,
  // Saturn, Uranus, and Neptune (131)
  const sumSatellites = parseInt(system.jupiter.satellites) + system.saturn.satellites 
  + system.uranus.satellites + system.neptune.satellites;
console.log("Sum of satellites:", sumSatellites);
  ...
  const momentumDifference = Math.abs(jupiterMomentum - plutoMomentum);
  console.log("Difference in momentum between Jupiter and Pluto:", momentumDifference);
